...
const InputUserRating = ({ studentid, israted, reservationid }: { studentid: string; israted: number; reservationid: string;}) => {
    const { user } = useAuth();
    ...
    const handleRating = async (value: number) => {
        try {
            const response = await fetch(`${URL}students/update-rating/${studentid}`, {
                method: 'PUT',
                headers: {
